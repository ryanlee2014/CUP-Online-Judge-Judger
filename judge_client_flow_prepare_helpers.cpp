...
void prepare_paths(JudgeContext &ctx, int runner_id, FlowState &state) {
    set_work_paths(runner_id, ctx.env, state);
    prepare_work_dir(ctx.config, state.work_dir);
    ctx.paths.work_dir = state.work_dir;
    load_language_cache(ctx.env);
...
void prepare_limits_and_policy(JudgeContext &ctx, FlowState &state) {
    ...
    ctx.language_model->setExtraPolicy(ctx.env.oj_home.c_str(), state.work_dir);
...
void set_work_paths(int runner_id, const JudgeEnv &env, FlowState &state) {
    state.global_work_dir = build_run_dir(runner_id, env);
